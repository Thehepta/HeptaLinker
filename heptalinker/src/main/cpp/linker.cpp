...
soinfo*  heptalinker_find_loadlibrary(const char * libname){
    ...
    for (soinfo* si : g_hepta_soinfo_list) {
        if(strncmp(libname,si->get_soname(), length)==0){
            return si;
        }
    }
    for (soinfo* si : g_system_soinfo_list) {
        if(strncmp(libname,si->get_soname(), length)==0){
            return si;
        }
    }
...
void LoadTask::soload( ) {
    ...
    for (const ElfW(Dyn)* d = get_elf_reader().dynamic(); d->d_tag != DT_NULL; ++d) {
        if (d->d_tag == DT_NEEDED) {
            ...
            soinfo* find_si = find_library(name);
            SymbolLookupLib SyLib = find_si->get_lookup_lib(true);
...
static bool realpath_fd(int fd, std::string* realpath) {
    ...
    sprintf(proc_self_fd, "/proc/self/fd/%d", fd);
